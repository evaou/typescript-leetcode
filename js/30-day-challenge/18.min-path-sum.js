"use strict";
/*
Given a m x n grid filled with non-negative numbers, find a path from top left to bottom right which minimizes the sum of all numbers along its path.

Note: You can only move either down or right at any point in time.

Example:

Input:
[
  [1,3,1],
  [1,5,1],
  [4,2,1]
]
Output: 7
Explanation: Because the path 1→3→1→1→1 minimizes the sum.
*/
function minPathSum(grid) {
    let leftValue;
    let topValue;
    let minValue;
    let hasNeighbor;
    for (let i = 0; i < grid.length; i++) {
        for (let j = 0; j < grid[0].length; j++) {
            topValue = Infinity;
            leftValue = Infinity;
            hasNeighbor = false;
            if (i - 1 >= 0) {
                topValue = grid[i - 1][j];
                hasNeighbor = true;
            }
            if (j - 1 >= 0) {
                leftValue = grid[i][j - 1];
                hasNeighbor = true;
            }
            minValue = 0;
            if (hasNeighbor) {
                minValue = Math.min(leftValue, topValue);
            }
            grid[i][j] += minValue;
        }
    }
    return grid[grid.length - 1][grid[0].length - 1];
}
let value = [
    [0, 2, 2, 6, 4, 1, 6, 2, 9, 9, 5, 8, 4, 4],
    [0, 3, 6, 4, 5, 5, 9, 7, 8, 3, 9, 9, 5, 4],
    [6, 9, 0, 7, 2, 2, 5, 6, 3, 1, 0, 4, 2, 5],
    [3, 8, 2, 3, 2, 8, 8, 7, 5, 9, 6, 3, 4, 5],
    [4, 0, 1, 3, 9, 2, 0, 1, 6, 7, 9, 2, 8, 9],
    [6, 2, 7, 9, 0, 9, 5, 2, 7, 5, 1, 4, 4, 7],
    [9, 8, 3, 3, 0, 6, 8, 0, 8, 8, 3, 5, 7, 3],
    [7, 7, 4, 5, 9, 1, 5, 0, 2, 2, 2, 1, 7, 4],
    [5, 1, 3, 4, 1, 6, 0, 4, 3, 8, 4, 3, 9, 9],
    [0, 6, 4, 9, 4, 1, 5, 5, 4, 2, 5, 7, 4, 0],
    [0, 1, 6, 6, 4, 9, 2, 7, 8, 2, 1, 3, 3, 7],
    [8, 4, 9, 9, 2, 3, 8, 6, 6, 5, 4, 1, 7, 9],
];
console.log(minPathSum(value)); // 63
value = [
    [1, 3, 1],
    [1, 5, 1],
    [4, 2, 1],
]; // 7
console.log(minPathSum(value));
//# sourceMappingURL=18.min-path-sum.js.map